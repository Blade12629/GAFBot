...
namespace ServerModule.Network.Packets
{
    public class PacketWriter
    {
        ...
        public void Write(TimeSpan t, int index = -1)
        {
            Write(t.Hours, index);
            Write(t.Minutes, index == -1 ? -1 : index + 4);
            Write(t.Seconds, index == -1 ? -1 : index + 8);
        }
        ...
        public void Write(string v, int index = -1)
        {
            ...
            Write(index == -1 ? -1 : index + 4, _encoding.GetBytes(v));
        }

        public byte[] Build()
        {
            byte[] data = new byte[_PACKET_HEADER_LENGTH + _data.Count];
            data = data.Replace(BitConverter.GetBytes(_data.Count), 0);
            data = data.Replace(new byte[] { Packet.Id }, 4);
            data = data.Replace(_data.ToArray(), 5);
